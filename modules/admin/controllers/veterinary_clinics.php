...
/**
 * Description of vetenary_clinics
 *
 * @author kofoworola
 */
class veterinary_clinics extends MX_Controller {
    ...
    function index() {
        $data['clinics'] = $this->clinic_model->get_all_clinics();
        $data['fields'] = $this->custom_field_model->get_custom_fields(1);
        $data['page_title'] = lang('clinics');
    ...
    function add() {
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            $users = $this->clinic_model->get_all_clinics();
            ...
            if ($this->form_validation->run() == true) {
                ...
                $save['user_role'] = 1;
                $save['user_type'] = "clinics";

                $p_key = $this->clinic_model->save($save);
                ...
                $this->email->send();
                $this->session->set_flashdata('message', lang('clinic_saved'));
    ...
    function edit($id = false) {
        if ($this->input->server('REQUEST_METHOD') === 'POST') {
            ...
            if ($this->form_validation->run()) {
                ...
                $this->clinic_model->update($save, $id);
                $business_name = $this->input->post('business_name');
                $save_business['name'] = $business_name;
                $save_business['doctor_id'] = $id;
                $this->setting_model->update_by_id($save_business,$id);
                $this->session->set_flashdata('message', lang('clinic_updated'));
    ...
    function delete($id = false) {
        if ($id) {
            ...
            $this->session->set_flashdata('message', lang('clinic_deleted'));
